...
LEADING_SEMESTER_PATTERN = re.compile(
    r"^\s*(1st|2nd|3rd|4th|5th|6th|7th|8th)\s+SEMESTER\b",
    re.IGNORECASE | re.MULTILINE,
)
EVEN_SEMESTER_TO_RANGE = {
    "2nd": "1st",
    "4th": "3rd",
    "6th": "5th",
    "8th": "7th",
...
def semester_key_from_body(body):
    text = body.get_text("\n", strip=True)
    # Prefer a real curriculum heading at the start of the block over a
    # buried "1st & 2nd Semester" mention in intro/cohort tables.
    leading = LEADING_SEMESTER_PATTERN.search(text)
    if leading:
        return leading.group(1).lower()
    match = INDIVIDUAL_SEMESTER_PATTERN.search(text)
    return match.group(1).lower() if match else None


def coalesce_semester_sections(sections):
    """Fold even-semester-only blocks into their 1st/3rd/5th/7th range keys.

    Some legacy calendars (e.g. Electrical 2014/2015) put 6th SEMESTER in a
    separate untitled container. That gets keyed as '6th' and would otherwise
    be dropped, since output only writes SEMESTER_ORDER odd ranges.
    """
    coalesced = OrderedDict()

    for key in SEMESTER_ORDER:
        if key in sections:
            coalesced[key] = sections[key]

    for even_key, range_key in EVEN_SEMESTER_TO_RANGE.items():
        if even_key not in sections:
            continue

        html = sections[even_key]
        plain = BeautifulSoup(html, "html.parser").get_text("\n", strip=True)
        leading = LEADING_SEMESTER_PATTERN.search(plain)
        # Skip junk blocks (cohort tables) that only mention the range in prose.
        if not leading or leading.group(1).lower() != even_key:
            continue

        if range_key in coalesced:
            coalesced[range_key] = coalesced[range_key] + html
        else:
            coalesced[range_key] = html
...
def extract_sections_from_soup(soup, range_hint=None):
    ...
    if blocks:
        ...
        if sections:
            return coalesce_semester_sections(sections)
...
def fix_relative_links(soup, calendar_base_url):
    """Resolve course links against the year calendar base, not site root.

    Older calendars often use hrefs like pg2598.html. Joining those to
    http://torontomu.ca produced broken URLs; they must become
    http://torontomu.ca/calendar/<year>-<year+1>/pg2598.html#...
    """
    broken_abs = re.compile(
        r"^https?://(?:www\.)?torontomu\.ca/(pg\d+\.html(?:#.*)?)$",
        re.IGNORECASE,
    )
    for tag in soup.find_all(href=True):
        ...
        if not href or href.startswith(("#", "mailto:")):
            continue
        m = broken_abs.match(href)
        if m:
            tag["href"] = urljoin(calendar_base_url, m.group(1))
            continue
        if href.startswith(("http://", "https://")):
            continue
        tag["href"] = urljoin(calendar_base_url, href)


def process_section_html(section_html, calendar_base_url):
    soup = BeautifulSoup(section_html, "html.parser")
    add_checkboxes(soup)
    fix_relative_links(soup, calendar_base_url)
    return soup.decode_contents()


def build_panel_html(heading_text, sections, calendar_base_url):
    body_parts = []
    for key in SEMESTER_ORDER:
        if key not in sections:
            continue
        ...
        processed = process_section_html(sections[key], calendar_base_url)
        body_parts.append(
...
def write_layout_file(file_name, heading_text, sections, calendar_base_url):
    combined_html = build_panel_html(heading_text, sections, calendar_base_url)
